...
/* 표준 vprintf() 함수로, printf()와 비슷하지만 va_list를 사용한다. */
int
vprintf (const char *format, va_list args) {
	return vhprintf (STDOUT_FILENO, format, args);
}

/* printf()와 같지만, 출력을 주어진 HANDLE에 쓴다. */
int
...
/* vhprintf_helper()용 보조 데이터. */
struct vhprintf_aux {
	char buf[64];       /* 문자 버퍼. */
	char *p;            /* 버퍼의 현재 위치. */
	int char_cnt;       /* 지금까지 기록한 총 문자 수. */
	int handle;         /* 출력 파일 HANDLE. */
};

static void add_char (char, void *);
static void flush (struct vhprintf_aux *);

/* printf()의 형식 지정자 FORMAT을 ARGS에 담긴 인자로 형식화하고, 출력을 주어진 HANDLE에 쓴다. */
int
...
/* AUX의 버퍼에 C를 추가하고, 버퍼가 가득 차면 비운다. */
static void
